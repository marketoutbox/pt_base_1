...
self.onmessage = async (event) => {
    ...
    try {
      ...
      if (modelType === "ols") {
        for (let i = 0; i < minLength; i++) {
          ...
          const currentPriceA =
            typeof data.pricesA[i].close === "string" ? Number.parseFloat(data.pricesA[i].close) : data.pricesA[i].close
          const currentPriceB =
            typeof data.pricesB[i].close === "string" ? Number.parseFloat(data.pricesB[i].close) : data.pricesB[i].close
        ...
        seriesForADF = distances
        seriesTypeForADF = "distances"
      }
      ...
      const practicalTradeHalfLife = calculatePracticalTradeHalfLife(
        zScores,
        data.params.entryThreshold,
        data.params.exitThreshold,
      )
      ...
      for (let i = 0; i < dates.length; i++) {
        ...
        if (modelType === "ols" || modelType === "kalman") {
          row.alpha = alphas[i]
          row.hedgeRatio = hedgeRatios[i]
          row.spread = spreads[i]
        } else if (modelType === "euclidean") {
          row.normalizedA = stockAPrices[i] / data.pricesA[0].close
          row.normalizedB = stockBPrices[i] / data.pricesB[0].close
      ...
      const dataForBands = seriesForADF // Use the series that was calculated for the model
      const rollingStatsWindow = modelType === "euclidean" ? euclideanLookbackWindow : olsLookbackWindow // Use appropriate lookback
  ...
  } else if (type === "runAdfTest") {
    self.postMessage({ type: "debug", message: "Received runAdfTest message." })
    try {
      await initializeWasm() // Ensure WASM is initialized before use

      const { series1, series2 } = data
      // Assuming series1 and series2 are arrays of numbers
      // For a real ADF test, you'd typically test the residual series (spread)
      // after regressing one series on the other.
      // For this example, let's just use series1 as the data to test for stationarity.
      // In a real pair trading scenario, you'd calculate the spread first.

      // Example: Calculate a simple spread for demonstration
      const cleanData = series1.map((val, i) => val - series2[i]).filter((val) => !isNaN(val))
      ...
      if (cleanData.length === 0) {
        self.postMessage({
          ...
          result: {
            ...
            is_stationary: false,
            error: "No valid data for ADF test.",
          },
        })
        return
      }
      ...
      const adfResult = get_adf_p_value_and_stationarity(statistic, cleanData.length)

      self.postMessage({ type: "adfResult", result: adfResult })
    } catch (error) {
      console.error("Error in ADF worker:", error)
      self.postMessage({
        type: "error",
        message: `Error performing ADF test: ${error.message || error}`,
      })
    }
  }
}
